...
const GENRE_COLORS: Record<string, string> = {
  ...
  comedie: 'bg-yellow-500/20 text-yellow-400',
  drame: 'bg-purple-500/20 text-purple-400',
  documentaire: 'bg-blue-500/20 text-blue-400',
  animation: 'bg-pink-500/20 text-pink-400',
  thriller: 'bg-orange-500/20 text-orange-400',
  autre: 'bg-white/10 text-beige/50',
};
...
export default function CinemaAdminPage() {
  const [allItems, setAllItems] = useState<CinemaArticle[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<CinemaArticle | null>(null);
  ...
  const openEdit = (item: CinemaArticle) => {
    setEditing(item);
    // Try to parse stored subtitle and genre from excerpt prefix if needed
    setForm({
      title: item.title,
      subtitle: '',
      category: 'cinema',
  ...
  const stats = {
    ...
    views: totalViews,
    featured: allItems.filter(i => i.isFeatured).length,
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-beige">Cinéma & Culture</h1>
              <p className="text-sm text-beige/40 mt-0.5">Contenus cinéma et culturels de la plateforme</p>
            </div>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Rechercher un article cinéma..."
        ...
        <div className="bg-[#141414] border border-white/5 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 text-beige/40">
              ...
              <p className="text-sm">Aucun article cinéma trouvé</p>
            </div>
          ) : (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 rounded-lg text-xs font-medium bg-purple-500/20 text-purple-400">
                            {item.category === 'cinema' ? 'Cinéma' : 'Culture'}
                          </span>
                        </td>
                        <td className="px-4 py-3">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-semibold text-beige">
                    {editing ? 'Modifier l\'article cinéma' : 'Ajouter un article cinéma'}
                  </h2>
                </div>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div>
                  <label className={lc}>Genre</label>
                  <select
                    value={form.genre}
                  ...
                  {form.genre && (
                    <div className="mt-2">
                      <span className={`px-2 py-0.5 rounded-lg text-xs font-medium ${GENRE_COLORS[form.genre] || 'bg-white/10 text-beige/50'}`}>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Contenu complet de l'article cinéma..."
                    rows={6}
                    className={`${ic} resize-none`}
                  />
                </div>
