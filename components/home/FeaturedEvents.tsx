      ...
      <div className="container-custom">
        ...
        {isLoading && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-pulse">
            <div className="lg:col-span-5 h-[500px] bg-night-50 rounded-3xl" />
            <div className="lg:col-span-7 grid grid-cols-2 gap-4">
              {[0,1,2,3].map(i => <div key={i} className="h-[230px] bg-night-50 rounded-2xl" />)}
            </div>
          </div>
        )}
        ...
        {!isLoading && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {displayEvents[0] && (
              ...
              >
                <Link href={`/events/${displayEvents[0].slug}`} className="group block h-full">
                  <div className="relative h-[500px] rounded-3xl overflow-hidden card-premium">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className={cn(
                        ...
                        eventCategoryLabels[displayEvents[0].category]?.color || 'bg-gold'
                      )}>
                        {eventCategoryLabels[displayEvents[0].category]?.[lang] || displayEvents[0].category}
                      </span>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      ...
                      <div className="flex items-center gap-4 text-white/70 text-sm">
                        <span className="flex items-center gap-1">
                          ...
                          {formatDate(displayEvents[0].date, lang)}
                        </span>
                        <span className="flex items-center gap-1">
                          <MapPin className="w-4 h-4 text-gold" />
                          {displayEvents[0].location}
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-4">
                        <span className="text-gold font-bold text-lg">
                          {displayEvents[0].isFree ? t('general.free') : `${displayEvents[0].ticketPrice?.toLocaleString()} XAF`}
                        </span>
            ...
            <div className="lg:col-span-7">
              ...
              >
                {displayEvents.slice(1, 5).map((event) => (
